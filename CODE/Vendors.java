...
class Vendors extends JPanel
{
	...
	JPanel DisplayPanel;
	JTextField Name,Address,PhoneNo,Email,Id;
	JButton Add,Reset;
	...
	Vendors()
	{
		...
		VendorData = getTableData();
		VendorTable = new JTable(VendorData,ColumnHeaders);
		add(new JScrollPane(VendorTable));
		setSize(500,500);
		setVisible(true);
	...
	public void PrepareDB()
	{
		try
		{
			...
			rs = stmt.executeQuery("select * from VENDORDETAILS");
			rsmd = rs.getMetaData();
			...
			public void actionPerformed(ActionEvent ae)
			{
				...
				}else
				{
					...
					try
					{
						...
						int i = stmt.executeUpdate("INSERT INTO VENDORDETAILS(VID,VNAME,VADDRESS,VPHONE,VEMAIL) VALUES('"+t1+"','"+t2+"','"+t3+"','"+t4+"','"+t5+"')");
						JOptionPane.showMessageDialog(null,"ENTERED NEW VENDOR");
						con.close();
					}catch(Exception e)
					{
						JOptionPane.showMessageDialog(null,"ERROR IN ENTERING NEW VENDOR");
					}
				}
			}
		});
